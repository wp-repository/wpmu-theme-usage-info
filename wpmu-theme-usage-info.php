...
class WPMU_Theme_Usage_Info {
	...
	function __construct() {
		...
		load_plugin_textdomain( 'wpmu-theme-usage-info', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	...
	function action_links($actions, $theme){
		...
		$text = "<div class='wpmu-theme-usage-info'>" . __( 'Used on', 'wpmu-theme-usage-info') . " ";
		if (sizeOf($blogs) > 0) {
			$text .='<a href="#TB_inline?height=155&width=300&inlineId='. $name . '" class="thickbox" title="' . __( 'Sites that use this theme', 'wpmu-theme-usage-info') . '">';
		}
		...
		if(sizeOf($blogs) > 0){
			...
			$text .=( __( 'Activated on the following sites:', 'wpmu-theme-usage-info') . " <ul>");
			foreach ($blogs as $blog){
	...
	function help_tabs() {
		...
		$screen->add_help_tab( array(
			...
			'title'     => __('About', 'wpmu-theme-usage-info'),
	...
	function about_tab() { ?>
		...
		<ul class="tab-about">
			<li><b><?php _e( 'Development', 'wpmu-theme-usage-info'); ?>:</b>
				<ul>
			...
			<li><b><?php _e( 'Languages', 'wpmu-theme-usage-info'); ?>:</b> English (development), German, Spanish, <a href="https://translate.foe-services.de/projects/cets-theme-info" target="_blank">more...</a></li> 
			<li><b><?php _e( 'License', 'wpmu-theme-usage-info'); ?>:</b> <a href="http://www.gnu.org/licenses/gpl-2.0.html" target="_blank">GPLv2 or later</a></li>
		</ul>
	<?php 
	}
	...
	function theme_info_page( $active_tab = '' ) {            
		...
		if (!$this->version_supported()) {
			echo "<div class='wrap'><h2>" . printf( __( 'Theme Usage Information %s This plugin requires at least WordPress version 3.4 - Please upgrade to stay safe and secure.', 'wpmu-theme-usage-info'), '</h2>') . "</div>";
			return;
		}
		//Handle updates
	   ...
	   <?php
		...
		<div class="wrap">
			...
			<h2 class="nav-tab-wrapper">
				<a href="?page=wpmu-theme-usage-info.php&tab=themes" class="nav-tab <?php echo $active_tab == 'themes' ? 'nav-tab-active' : ''; ?>"><?php _e( 'Themes', 'wpmu-theme-usage-info'); ?></a>
				<a href="?page=wpmu-theme-usage-info.php&tab=settings" class="nav-tab <?php echo $active_tab == 'settings' ? 'nav-tab-active' : ''; ?>"><?php _e( 'Settings', 'wpmu-theme-usage-info'); ?></a>
			</h2>
			...
			<?php if ($active_tab == 'settings') { ?>
				...
				<div class="tab-body">
					<table class="widefat" id="cets_active_themes">
						<thead>
							<tr>
								<th class="nocase"><?php _e( 'Used Themes', 'wpmu-theme-usage-info'); ?></th>
								<th class="case" style="text-align: center !important;"><?php _e( 'Activated Sitewide', 'wpmu-theme-usage-info'); ?></th>
								<th class="num"><?php _e( 'Total Sites', 'wpmu-theme-usage-info'); ?></th>
								<th><?php _e( 'Site Titles', 'wpmu-theme-usage-info'); ?></th>
							</tr>
						</thead>
						<tbody id="themes">
							<?php
							...
							foreach ( $list as $theme => $blogs ) {
								...
								?>
								<a href="javascript:void(0)" onClick="jQuery('#bloglist_<?php echo $counter; ?>').toggle(400);"><?php _e( 'Show/Hide Sites', 'wpmu-theme-usage-info'); ?></a>
								<?php
					...
					<table class="widefat">
						<thead>
							<tr>
								<th class="nocase"><?php _e( 'Unused Themes', 'wpmu-theme-usage-info'); ?></th>
								<th class="case" style="text-align: center !important;"><?php _e( 'Activated Sitewide', 'wpmu-theme-usage-info'); ?></th>
								<th class="num"><?php _e( 'Total Sites', 'wpmu-theme-usage-info'); ?></th>
								<th>&nbsp;</th>
							</tr>
	...
	function load_scripts( $screen ) {
		if ( 'themes_page_wpmu-theme-usage-info-network' == $screen->id ) {
...
$wpmu_theme_usage_info = WPMU_Theme_Usage_Info;
